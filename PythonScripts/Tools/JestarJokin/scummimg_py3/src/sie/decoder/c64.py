...
def unpackV1Object(objectMap, charMap, colours, width, height):
    ...
    for hstrip_i in range(num_blocks):
        for vstrip_i in range(num_strips): # x is equivalent to stripnr
...
def unpackV1ObjectMaskData(objectMap, maskCharMap, width, height):
    ...
    # For each column strip number
    for hstrip_i in range(num_blocks):
        for vstrip_i in range(num_strips): # equivalent to stripnr
...
def unpackV1Background(charMap, picMap, colourMap, colours, width, height):
    ...
    dstPitch = width
    # For each column strip number
    for x in range(num_strips): # x is equivalent to stripnr
        # For every 8th row
        col_start = x * 8
