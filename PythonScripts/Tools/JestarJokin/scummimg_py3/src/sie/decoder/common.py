...
class ImageDecoderBase(ImageCodecBase):
    ...
    def readDimensions(self, lflf_path):
        ...
        if self.config.bitmap_path[0] == "SMAP.dmp":
            return header_reader.getDimensionsObject(header_path)
        else:
            return header_reader.getDimensionsRoom(header_path)

    def readPalette(self, lflf_path, palette_num):
        palette_path = self.getExistingPalettePath(lflf_path, palette_num)
        palf = open(palette_path, 'rb')
        pal = None
        try:
