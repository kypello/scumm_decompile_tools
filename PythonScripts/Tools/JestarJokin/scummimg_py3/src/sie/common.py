...
class HeaderReaderWriterBase(object):
    ...
    def getDimensionsRoom(self, header_path):
        return None, None
    
    def getDimensionsObject(self, header_path):
        return None, None

    def updateHeaderRoom(self, header_path, width, height):
        pass
    
    def updateHeaderObject(self, header_path, width, height):
        pass
...
class HeaderReaderWriterXml(HeaderReaderWriterBase):
    ...
    def getDimensionsObject(self, header_path):
        root = self.getHeaderData(header_path)
        width = int(root.find("image").find("width").text)
        height = int(root.find("image").find("height").text)
        return width, height
    ...
    def updateHeaderObject(self, header_path, width, height):
        root = self.getHeaderData(header_path)
        root.find("image").find("width").text = str(width)
        root.find("image").find("height").text = str(height)
        et.ElementTree(root).write(header_path)
