...
class EncoderV1(common.ImageEncoderBase):
    ...
    def findCommonValuesForRLE(self, data):
        ...
        value_freqs_sorted = sorted(list(value_freqs.items()), key=lambda x: x[0])
    ...
    def getCommonColoursV1(self, source_data, width, height):
        ...
        colour_freqs_sorted = sorted(list(colour_freqs.items()), key=lambda x: x[1], reverse=True)
    ...
    def packBackgroundData(self, source_data, width, height):
        ...
        for vstrip_i in range(num_strips):
            for hstrip_i in range(num_blocks):
    ...
    def packBackgroundMaskData(self, source_data, width, height):
        ...
        for vstrip_i in range(num_vstrips):
            for hstrip_i in range(num_hstrips):
