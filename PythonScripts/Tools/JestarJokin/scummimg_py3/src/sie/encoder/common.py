...
class ImageEncoderBase(ImageCodecBase):
    ...
    def writeHeader(self, lflf_path, width, height):
        ...
        if self.config.bitmap_path[0] == "SMAP.dmp":
            header_rw.updateHeaderObject(header_path, width, height)
        else:
            header_rw.updateHeaderRoom(header_path, width, height)
...
class ImageEncoderVgaBase(ImageEncoderBase):
    def writeBitmap(self, lflf_path, image_path, source_image, width, height, compression_method, freeze_palette, quantization):
        ...
        # Write strips
        newsmapfile = open(self.getNewBitmapPath(lflf_path), 'wb')
        # Write dummy header
        newsmapfile.write(b'00000000')
        blockStart = newsmapfile.tell()
        numStrips = int(width / 8)
        ...
        for stripNum in range(numStrips):
            newsmapfile.write(b'\x01') # tell it it's an uncompressed thingo
